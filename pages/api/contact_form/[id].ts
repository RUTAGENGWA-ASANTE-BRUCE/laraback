...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await contact_form.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await contact_form.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await contact_form.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
