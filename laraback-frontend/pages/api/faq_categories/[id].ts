...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await faq_category.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await faq_category.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await faq_category.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
