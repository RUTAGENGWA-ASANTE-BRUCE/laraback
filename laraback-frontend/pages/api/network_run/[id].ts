...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await network_run.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await network_run.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await network_run.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
