...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await postback_log.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await postback_log.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await postback_log.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
