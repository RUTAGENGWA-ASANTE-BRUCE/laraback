...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await email_template.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await email_template.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await email_template.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
