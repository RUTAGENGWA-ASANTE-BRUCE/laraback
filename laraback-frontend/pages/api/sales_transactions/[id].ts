...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await sales_transaction.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await sales_transaction.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await sales_transaction.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
