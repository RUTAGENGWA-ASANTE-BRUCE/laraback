...
export default async function (req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    case "GET":
      try {
        const document = await missing_cb_claim.findById(id);
        if (!document)
    ...
    case "DELETE":
      try {
        const deleteDocument = await missing_cb_claim.findByIdAndDelete(id);
        if (!deleteDocument)
          return res.status(404).json({ msg: "The document does not exist" });
    ...
    case "PUT":
      try {
        const updatedDocument = await missing_cb_claim.findByIdAndUpdate(
          id,
          body,
          {
            new: true,
